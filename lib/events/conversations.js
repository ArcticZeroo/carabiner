...
const Conversation = require('../structures/conversation/Conversation');
...
class ConversationEventHandler extends EventHandler {
    listen() {
         ...
         */
        this.emitter.on(this.onCreate, 'channel_created', 'im_created');
         ...
         */
        this.emitter.on(this.onCreate, 'group_open', 'im_open');
    ...
    onSelfJoin(data) {
        ...
        if (this.client.conversations.has(id)) {
            this.client.retrieveConversation(id)
                .then(conversation => this.emitWithSubtype('selfJoin', conversation))
