...
class Message{
    ...
    setup(data) {
         ...
         * @type {Channel}
         */
        this.channel = this.client.channels.find('id', data.channel);
        ...
        this.reactions = (data.reactions || []).map((r)=> {
            r.users = r.users.map((id => this.client.users.find('id', id)));
            return r;
        });
         ...
         * @type {Array<Channel>}
         */
        this.pinnedTo = (data.pinnedTo || []).map((id)=> this.client.channels.find('id', id));
