...
module.exports = class Client extends EventEmitter {
     ...
     * @param opts {object} [{}] - Options to use.
     * @param opts.rtm {boolean} [true] - Whether the client should connect to RTM.
     * @param opts.getFullChannelInfo {boolean} [false] - Whether the client should retrieve ALL info about channels. This will significantly increase startup time if true, and could hit rate limit.
     * @param opts.separateGroupAndMpim {boolean} [false] - Whether group and mpim channels should have a different type.
     * @param opts.getUserPresence {boolean} [false] - Whether user preference should be added to cached data. This will significantly increase startup time if true.
     */
    ...
    async _getTeamInfo() {
        ...
        try {
            response = this.api.methods.team.info();
        } catch (e) {
            throw e;
        }

        // We already made an instance, so just set it up now.
        this.team.setup(response.team);
    }
