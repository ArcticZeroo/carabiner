...
class Client extends EventEmitter {
     ...
     * @param {boolean} [opts.getFullChannelInfo=false] - Whether the client should retrieve ALL info about conversations. This will significantly increase startup time if true, and could hit rate limit.
    ...
    async _populateChannelInfo(chats) {
        if (!chats || !chats.length) {
            try {
                chats = await SlackUtil.getPages({ method: this.api.methods.conversations.list, getData: (r) => r.conversations });
            } catch (e) {
                throw e;
            }
        }
