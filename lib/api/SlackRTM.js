...
const SlackError = require('../enum/SlackError');
const PromiseUtil = require('../util/PromiseUtil');
const { rtm: config } = require('../../config/api');
...
class SlackRTM extends EventEmitter{
     ...
     */
    constructor({ api, retryIfMigrating = true, migrationRetryAttempts = Number.POSITIVE_INFINITY }) {
        super();
     ...
     * @return {Promise.<void>}
     */
    async connect(url, retryCount = 0) {
        if (url) {
